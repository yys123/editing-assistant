...
class DeepSeekClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deepseek_disabled_thinking_mode_sends_sampling_params(self):
        fake_client = _FakeAsyncClient(
            {
        ...
        self.assertEqual(payload["presence_penalty"], 1.0)
        self.assertEqual(payload["frequency_penalty"], 1.0)
