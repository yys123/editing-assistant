...
- 例如“本指南”“本共识”“专家组认为”“我们认为”等禁用词，只有完整短语真实出现在章节原文中才可报告；如果只出现在评审标准、内容规范或参考指南中，不得报告为原文问题。
- “2025 CSCO 指南推荐”“2025版CSCO指南推荐”“XX指南指出/建议/推荐”“XX共识指出/建议/推荐”等具体来源归属表达不属于上述禁用词；不得仅因包含“指南”“共识”“指出”“推荐”等字样而报告禁用词问题，也不得只为规避禁用词要求改写具体指南名称。"""

FORBIDDEN_SELF_REFERENCE_PHRASES = ("本指南", "本共识", "专家组认为", "我们认为")
FORBIDDEN_SELF_REFERENCE_CLAIM_HINTS = (
    "禁用词",
    "禁止用语",
    "禁止词",
    "不得保留",
    "不应保留",
    "保留",
    *FORBIDDEN_SELF_REFERENCE_PHRASES,
)
...
def _contains_forbidden_self_reference(text: str) -> bool:
    return any(phrase in (text or "") for phrase in FORBIDDEN_SELF_REFERENCE_PHRASES)


def _claims_forbidden_self_reference(issue: SectionIssue) -> bool:
    text = " ".join([
        issue.description or "",
        *(issue.examples or []),
        *(anchor.quote for anchor in issue.anchors if anchor.quote),
    ])
    if not any(hint in text for hint in FORBIDDEN_SELF_REFERENCE_CLAIM_HINTS):
        return False
    return any(
        marker in text
        for marker in ("禁用词", "禁止用语", "禁止词", *FORBIDDEN_SELF_REFERENCE_PHRASES)
    )


def _drop_false_forbidden_self_reference_issues(
    issues: List[SectionIssue],
    section_content: str,
) -> List[SectionIssue]:
    if _contains_forbidden_self_reference(section_content):
        return issues

    return [
        issue for issue in issues
        if issue.issue_type != "style" or not _claims_forbidden_self_reference(issue)
    ]
...
- 合格阈值：累积＜20分/万字；优秀：累积＜10分/万字
- 扣分项：重复性文本/同一观点反复表达；研究性内容未精简（试验性/争议内容须精简，里程碑式可不精简）；必要图表缺失或不必要图表多余；必要超链缺失或多余；英文缩写首次出现未给出中英文全称；标题出现英文缩写；标题超过20字；保留"本指南""本共识"等完整自指字眼；机翻痕迹；错别字；格式不一致；专有名词不统一。具体指南或共识归属表达如“2025 CSCO 指南推荐”“XX指南指出”不是禁用词。
...
- style同类问题每处0.5分（英文缩写格式/标题问题/专有名词不统一/重复文本/机翻/完整自指禁用短语如"本指南"等）；合格线累积<20/万字，优秀<10/万字；具体指南或共识归属表达如“2025 CSCO 指南推荐”“XX指南指出”不是禁用词
...
async def analyze_section(
    ...
    if len(section.content) > CHUNK_THRESHOLD:
        ...
        merged_issues = _drop_false_forbidden_self_reference_issues(merged_issues, section.content)
    ...
    verified_issues = _drop_false_forbidden_self_reference_issues(verified_issues, section.content)
