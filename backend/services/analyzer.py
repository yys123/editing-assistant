...
- 合格阈值：重点扣分＜10分/万字，或累积＜20分/万字；优秀：重点＜8，或累积＜15
...
- 合格阈值：累积＜20分/万字；优秀：累积＜10分/万字（结构问题标准较严）
- 扣分参考：结构严重错位3分，小调整1分
...
{{
  ...
  "dim_four": {{
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="legacy_quality_eval")
...
- representative_questions每类取2-3个最有代表性的"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="legacy_user_needs")
    data = extract_json(text)
...
若所有问题均被推翻，输出 {{"verification_summary": "...", "issues": []}}"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="section_issue_verify")
    data = extract_json(text)
...
- missing_content重点内容：1-5分（重大缺失如完全没有诊断/治疗内容5分；影响临床使用3分；一般缺失1分）；is_key_content=true；合格线重点<20/万字，优秀<10/万字
...
- structure临床思维大问题（章节严重错位/分类字段放错）：3分；小结构调整：1分；合格线累积<20/万字，优秀<10/万字（结构标准严格）
...
- covered_in_kb根据问题本身判断，不确定时填false"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="needs_classify_batch")
    data = extract_json(text)
...
- covered_in_kb：若任何批次标记为false则取false"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="needs_merge_clusters")
    data = extract_json(text)
    return [NeedCluster(**c) for c in data.get("clusters", [])]
...
每个章节必须出现在mappings中（即使cluster_topics为空也要包含）。"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="needs_map_to_sections")
    data = extract_json(text)
...
"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="section_needs_gap")
    data = extract_json(text)
...
- 若建议新增章节，说明插入位置（例如：建议在「诊断」与「治疗」之间新增「XXX」章节）"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="needs_gap_placement")
    data = extract_json(text)
...
clusters聚类5-10个主要需求类型，按频次降序。"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="legacy_full_analysis")
    data = extract_json(text)
...
注意：rejected的条目不纳入计划；同一子章节的多个问题可合并为一条；按章节顺序输出。"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="plan_from_gap")
    data = extract_json(text)
...
注意：被专家标记为rejected的问题已从评估中移除，不需要纳入计划。专家补充或确认的问题应优先考虑。"""
    ...
    text = await generate_text(prompt, SYSTEM_PROMPT, context="legacy_iteration_plan")
    data = extract_json(text)
